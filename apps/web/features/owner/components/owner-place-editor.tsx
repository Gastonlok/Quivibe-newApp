...
export function OwnerPlaceEditor({
  ...
  function setResult(result: { success: boolean; error?: string }, successMessage: string) {
    setFeedback(
      result.success
  ...
  function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(() => {
      void updateOwnerPlaceAction({ placeId: place.id, ...form }).then((result) =>
        setResult(result, "Vos modifications ont ete enregistrees."),
      );
    });
  }
  ...
  function handleImageUrl(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    startTransition(() => {
      void addOwnerPlaceImageAction({ placeId: place.id, url: imageUrl, altText: imageAlt }).then((result) => {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-soft">
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-800">Telephone</span>
              <input
                type="tel"
                value={form.phone}
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-800">Gamme de prix</span>
              <select
                value={form.priceRange}
                onChange={(event) => setForm({ ...form, priceRange: Number(event.target.value) })}
                className={inputClass}
              >
                {[1, 2, 3, 4].map((value) => (
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-800">Latitude</span>
              <input
                type="number"
                step="any"
                value={form.latitude}
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-800">Longitude</span>
              <input
                type="number"
                step="any"
          ...
          <fieldset className="mt-6">
            <legend className="text-sm font-bold text-gray-800">Categories</legend>
        ...
        <details className="overflow-hidden rounded-3xl border border-primary-100 bg-white shadow-soft">
          ...
          <div className="p-6">
            <label className="flex cursor-pointer items-center justify-between gap-4 rounded-2xl bg-primary-50 p-4">
              <div>
                <p className="font-extrabold text-gray-950">Accepter les reservations</p>
                <p className="mt-1 text-sm text-gray-600">Le module de reservation apparaitra sur votre fiche publique.</p>
              </div>
              <input
                type="checkbox"
                checked={form.reservationsEnabled}
                onChange={(event) => setForm({ ...form, reservationsEnabled: event.target.checked })}
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Debut des reservations</span>
                <input
                  type="time"
                  value={form.reservationStartTime}
                  onChange={(event) => setForm({ ...form, reservationStartTime: event.target.value })}
                  className={inputClass}
                  required
                />
              </label>
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Fin des reservations</span>
                <input
                  type="time"
                  value={form.reservationEndTime}
                  onChange={(event) => setForm({ ...form, reservationEndTime: event.target.value })}
                  className={inputClass}
                  required
                />
              </label>
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Intervalle entre les creneaux</span>
                <select
                  value={form.reservationInterval}
                  onChange={(event) => setForm({ ...form, reservationInterval: Number(event.target.value) })}
                  className={inputClass}
                >
              ...
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Duree d'une reservation</span>
                <select
                  value={form.reservationDuration}
                  onChange={(event) => setForm({ ...form, reservationDuration: Number(event.target.value) })}
                  className={inputClass}
                >
              ...
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Capacite par creneau</span>
                <input
                  type="number"
                  min={1}
                  max={500}
                  value={form.reservationCapacity}
                  onChange={(event) => setForm({ ...form, reservationCapacity: Number(event.target.value) })}
                  className={inputClass}
                  required
                />
              </label>
              <label className="block">
                <span className="text-sm font-bold text-gray-800">Maximum par reservation</span>
                <input
                  type="number"
                  min={1}
                  max={50}
                  value={form.maxPartySize}
                  onChange={(event) => setForm({ ...form, maxPartySize: Number(event.target.value) })}
                  className={inputClass}
                  required
                />
              </label>
            ...
            <label className="mt-5 flex cursor-pointer items-center gap-3 text-sm font-bold text-gray-800">
              <input
                ...
                onChange={(event) => setForm({ ...form, autoConfirmReservations: event.target.checked })}
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-soft">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-primary-700">Galerie</p>
            <h2 className="mt-1 text-2xl font-extrabold tracking-tight text-gray-950">Vos images</h2>
            <p className="mt-1 text-sm text-gray-600">La premiere image est utilisee comme visuel principal de votre fiche.</p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {place.media.map((media, index) => (
            <article key={media.id} className="group overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
              <div className="relative aspect-[4/3] bg-gray-100">
                <img src={media.url} alt={media.altText || place.name} className="h-full w-full object-cover" />
                {index === 0 && (
              ...
              <div className="flex items-center justify-between gap-3 p-3">
                <p className="truncate text-sm font-bold text-gray-700">{media.altText || "Image de l'etablissement"}</p>
                <button
                  type="button"
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <form onSubmit={handleImageUpload} className="rounded-2xl border border-dashed border-primary-300 bg-primary-50/60 p-5">
            <div className="flex items-center gap-3 text-primary-800">
              <Upload className="h-5 w-5" />
              <h3 className="font-extrabold">Envoyer un fichier</h3>
            </div>
            <p className="mt-2 text-sm leading-6 text-gray-600">JPG, PNG ou WebP, 10 Mo maximum. Requiert la configuration Cloudinary.</p>
            <input name="image" type="file" accept="image/*" className="mt-4 block w-full text-sm text-gray-700" required />
            <input name="altText" placeholder="Description de l'image (optionnel)" className={inputClass} />
            <button type="submit" disabled={isPending} className="mt-4 inline-flex items-center gap-2 rounded-full bg-primary-600 px-4 py-2.5 text-sm font-extrabold text-white hover:bg-primary-700 disabled:opacity-60">
              <ImagePlus className="h-4 w-4" /> Envoyer l'image
            </button>
          </form>

          <form onSubmit={handleImageUrl} className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
            <div className="flex items-center gap-3 text-gray-900">
              <Link2 className="h-5 w-5 text-primary-700" />
              <h3 className="font-extrabold">Ajouter par lien</h3>
            </div>
            <p className="mt-2 text-sm leading-6 text-gray-600">Utilisez une URL directe vers une image deja hebergee.</p>
            <input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} type="url" placeholder="https://..." className={inputClass} required />
            <input value={imageAlt} onChange={(event) => setImageAlt(event.target.value)} placeholder="Description de l'image (optionnel)" className={inputClass} />
            <button type="submit" disabled={isPending} className="mt-4 inline-flex items-center gap-2 rounded-full border border-primary-600 bg-white px-4 py-2.5 text-sm font-extrabold text-primary-700 hover:bg-primary-50 disabled:opacity-60">
