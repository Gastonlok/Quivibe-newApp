...
const items = [
  ...
  { href: "/owner/analytics", label: "Statistiques", icon: BarChart3 },
  {
    ...
    label: "Planning et réservations",
    icon: CalendarCheck2,
  },
];
...
export function OwnerSidebar() {
  ...
  return (
    <aside className="border-b border-gray-200 bg-white lg:sticky lg:top-[4.5rem] lg:h-[calc(100vh-4.5rem)] lg:self-start lg:border-b-0 lg:border-r">
      <div className="flex h-full flex-col p-4 lg:p-6">
        ...
        >
          ...
          <span>
            <span className="block text-lg font-extrabold">Quivibe Pro</span>
        ...
        <nav className="mt-5 flex gap-2 overflow-x-auto lg:flex-col">
          {items.map(({ href, label, icon: Icon }) => {
            const active =
              pathname === href ||
              (href === "/owner/dashboard" &&
                pathname.startsWith("/owner/places"));
            return (
              <Link
                key={href}
                href={href}
        ...
        <p className="mt-auto hidden px-2 pt-8 text-xs leading-5 text-gray-500 lg:block">
          Pilotez vos fiches, reservations et performances depuis un seul
          espace.
        </p>
      </div>
    </aside>
